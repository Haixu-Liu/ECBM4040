    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                mode = 'train'
                x, cache['bn_{}'.format(i)] = bn_forward(x, gamma, beta, bn_params[i], mode)
            ###############################################
            ...
            if dropout_config['enabled']:
                x, cache['dropout_{}'.format(i)] = dropout_forward(x, dropout_config, mode='train')
            ###############################################
            # END OF DROPOUT                              #
            ###############################################
    ...
    def predict(self, X):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                beta = params["bn_beta_{}".format(i)]
                mode = 'test'
                x, _ = bn_forward(x, gamma, beta, bn_params[i], mode)
            ###############################################
            # END OF BATCH NORMALIZATION                  #
            ############################################### 
